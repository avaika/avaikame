...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        book_name_ru = soup.find('title').contents[0][:-12]
        author_ru_raw = soup.find('span', attrs={'data-wikidata-property-id': 'P50'}).contents[0]
        author_name_ru = author_ru_raw.contents[0]
        author_link_ru = "https://ru.wikipedia.org" + author_ru_raw['href']
        try:
            book_genre = soup.find('span', attrs={'data-wikidata-property-id': 'P136'}).text
        except:
            book_genre = 'default'
        try:
        ...
        result = BeautifulSoup(author_en, 'html.parser')
