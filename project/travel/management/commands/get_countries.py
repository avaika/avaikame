...
class Command(BaseCommand):
    def handle(self, *args, **options):
        req = urlopen("http://en.wikipedia.org/w/api.php?action=parse&page=Gallery_of_sovereign_state_flags&format=json&prop=text")
        page_json = json.load(req)
        page = BeautifulSoup(str(page_json["parse"]["text"]), 'html.parser')

        flag_dir = settings.MEDIA_ROOT + "/flags"
        if not os.path.exists(flag_dir):
            os.makedirs(flag_dir)

        for img in page.find_all('img', attrs={'alt': re.compile('Flag of.*')}):
            ...
            if created or country.flag != name:
                ...
                im_file.write(urlopen("https:" + img.get('src')).read())
                im_file.close()
                country.save()
